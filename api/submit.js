export default async function handler(req, res) {
  ...
  if (!NOTION_TOKEN || !DATABASE_ID) { res.status(500).json({ error: '環境變數未設定' }); return; }
  ...
  const props = {
    ...
    "初診諮詢":     { rich_text: [{ text: { content: d.newpt_summary || '' } }] },
    "實收金額":     { number: d.revenue || 0 },
  };
  const slotKeys = ['明天美顏針空檔','明天針刀空檔','後天美顏針空檔','後天針刀空檔'];
  slotKeys.forEach(k => { if (d.slots && d.slots[k]) props[k] = { select: { name: d.slots[k] } }; });
